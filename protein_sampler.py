...
class Property_RadiusOfGyration:
    def __init__(
        ...
        for frame in frame_list:
            ...
            self.rgyr.append(protein_data.trajectory_data.select_atoms(
...
class Bhatta_Distance:
    ...
    def __init__(self, Prop_vector, verbose=True):
        ...
        freq_prop_vector = discretize_to_dict(
            Prop_vector, min_value, max_value)

        # test - the distance should be zero (or close to zero) on itself
        sample_size = len(freq_prop_vector)
        b_distance = dictances.bhattacharyya(
            freq_prop_vector, freq_prop_vector)
        ...
        for sample_size in [10, 50, 100, 200, 400, 500]:
            ...
            b_distance = dictances.bhattacharyya(
                freq_prop_vector, freq_sub_prop_vector)
            print(
                "size: {0:6d} distance: {1:.3f}".format(
                    sample_size, b_distance))
...
class KL_diver:
    def __init__(self, Prop_vector, verbose=True):
        ...
        # discretisation of the original vector with all values
        KL_freq_prop_vector = discretize_to_dict(
            Prop_vector, min_value, max_value)
        ...
        kl_pq_distance = dictances.kullback_leibler(
            KL_freq_prop_vector_clean, KL_freq_prop_vector_clean)
        ...
        for sample_size in [10, 50, 100, 200, 400, 500]:
         # for sample_size in [800,4000,8000,16000,32000,40000]:
            ...
            # discretisation of the subsampled vector
            KL_freq_sub_prop_vector = discretize_to_dict(
                KL_sub_prop_vector, min_value, max_value)

            KL_freq_sub_prop_vector_clean = replace_zero(
                KL_freq_sub_prop_vector)
            # calculate the kl divergence
            kl_pq_distance = dictances.kullback_leibler(
                KL_freq_sub_prop_vector_clean, KL_freq_sub_prop_vector_clean)
...
class Pearson_dict:
    def __init__(self, Prop_vector, verbose=True):
        ...
        kl_pq_distance = dictances.pearson(
            KL_freq_prop_vector, KL_freq_prop_vector)
        ...
        for sample_size in [10, 50, 100, 200, 400, 500]:
         # for sample_size in [800,4000,8000,16000,32000,40000]:
            ...
            # calculate the kl divergence
            kl_pq_distance = dictances.pearson(
                KL_freq_sub_prop_vector, KL_freq_sub_prop_vector)
...
class Property_PCA_analysis:
    ...
    def __init__(
        ...
        full_pca = pca.PCA(
            protein_data.trajectory_data,
            select='name CA',
            align=False,
            mean=None).run()
        print("PCA TYPE ", type(full_pca))
        full_trans = full_pca.transform(calpha_pca, n_components=5)
        # Convert full transformed pca into pandas dataframe
        df1 = pd.DataFrame(
            full_trans, columns=[
                'PC{}'.format(
                    i + 1) for i in range(5)])
        df1['Time (ps)'] = df1.index * \
            protein_data.trajectory_data.trajectory.dt
        print(df1.head())
...
class Subsampled_PCA_analysis:
    def __init__(
        ...
        coordinates = np.empty(
            (len(sliced_traj),
             protein_data.trajectory_data.select_atoms('name CA').n_atoms,
             3),
            dtype=np.float32)
        for i, ts in enumerate(sliced_traj):
            coordinates[i] = protein_data.trajectory_data.select_atoms(
                atom_selection).positions
        ...
        # use the subsampled universe u2 (should also be faster because its in
        # memory)
        ...
        # Convert sampled pca into pandas dataframe
        df = pd.DataFrame(
            sampled_transformed, columns=[
                'PC{}'.format(
                    i + 1) for i in range(5)])
        ...
        plt.plot(sampled_pca.cumulated_variance[:5])
        print("Graph for Subsampled PCA")
        sns.lmplot(x="PC1", y="PC2",
                   data=df,
                   fit_reg=False,
                   legend=True
...
def Property_RMSF(protein_data, frame_list):
    ...
    for frame in frame_list:
        protein_data.trajectory_data.trajectory[frame]
        R = rms.RMSF(
            protein_data.trajectory_data.select_atoms('name CA')).run()
        return R.rmsf
...
def main():
    ...
    # ploting radius of Gyration and saving as PNG in root directory
    save_output_plot(rgyr_vector)
    ...
    # stored in the dictionary
        sampled_rmsd_vector = Property_RMSD(
            pro_data, frame_sampler.sampled_frame_list).rmsd

        hist_sampled_rmsd = plot_histogram(sampled_rmsd_vector)
        box_sampled_rmsd = plot_boxplot(sampled_rmsd_vector)

        sampled_rmsf_vector = Property_RMSF_sampled(
            pro_data, frame_sampler.sampled_frame_list)

        sampled_rgyr_vector = Property_RadiusOfGyration(
            pro_data, frame_sampler.sampled_frame_list)

        save_output_plot(sampled_rgyr_vector)

        sampled_pca_vector = Subsampled_PCA_analysis(
            pro_data, frame_sampler.sampled_frame_list)

        pro_data.add_property(
            sampled_rmsd_vector,
            "SAMPLED RMSD",
            "random" + str(size))
    ...
    pro_data.statistical_analysis(
        rgyr_vector.rgyr,
        sampled_rgyr_vector.rgyr,
        "RGYR")
